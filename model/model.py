...
class NetX2Y(torch.nn.Module):
    def __init__(self, hidden1, hidden2, hidden3, hidden4, physical_hidden, is_physical_info, n_feature, n_output):
        super(NetX2Y, self).__init__()
        self.is_physical_info = is_physical_info
        self.hidden1 = torch.nn.Linear(n_feature, hidden1)
        if self.is_physical_info:
            self.hidden2 = torch.nn.Linear(hidden1 + physical_hidden, hidden2)   # 输出层线性输出
        else:
        ...
        self.predict = torch.nn.Linear(hidden4, n_output)
