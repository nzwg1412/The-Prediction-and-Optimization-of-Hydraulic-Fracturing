...
def main_func_train_val(hidden1, hidden2, hidden3, hidden4, hidden_feat_len, out_feat_len, Hidden1, Hidden2, Hidden3,
                        Hidden4, n_feature_len, noise, H_model=1, add_physical_info=1):
    # load data
    global ERROR
    ...
    if H_model == 0:
        if add_physical_info:
            ...
            train_writer_x2y = SummaryWriter(
                os.path.join(runs_save_folder, trainer_name + '_x2y_noise_%.4f' % noise))
            save_x2y_model_path = model_save_folder + "/%s_best_model_x2y_noise_%.4f.pth" % (trainer_name, noise)
